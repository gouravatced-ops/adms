...
<style>
    ...
    .premium-container {
        padding: 1.5rem;
        width: 100%;
        max-width: 100%; /* Removing the 1200px restriction */
        margin: 0 auto;
...
<div class="premium-container" id="source-container">
    <div class="premium-card">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            <h2 class="page-title" style="margin-bottom: 0;">Scanner Property Search</h2>
            <button id="toggle-magnifier" class="btn-magnifier-header" title="Toggle Magnifier">
                <i class="fas fa-search-plus"></i> Magnifier
            </button>
        </div>
        
        <form action="{{ route('scanner.property.search.submit') }}" method="POST">
            @csrf
            <div class="search-group">
                <div class="input-wrapper">
                    ...
                    <input type="text" name="property_number" id="property_number" 
                           ...
                           class="search-input" 
                           placeholder="Enter Property Number (e.g. C-40)" required>
                </div>
            ...
            <div>
                @foreach($allottees as $index => $item)
                    @php
                        ...
                        $hasMasterDocument = $item->masterDocuments && $item->masterDocuments->isNotEmpty();
                        $isCurrentAllottee = ($index === 0); // First item is the most recent due to DESC order
                    @endphp
                        ...
                        <div class="result-header">
                            ...
                            <div>
                                @if(!$hasMasterDocument)
                                    <a href="{{ url('applicant/nametransfer/master/documents/' . encrypt($item->id)) }}" class="btn-danger">
                                        <i class="fas fa-upload" style="margin-right: 4px;"></i> Master File
                                    </a>
                                @else
                                    <span class="badge-success">
                                        <i class="fas fa-check"></i> Master Uploaded
                                    </span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="grid-container">
                            ...
                            <div class="detail-box box-emi">
                                ...
                                <ul class="detail-list">
                                    ...
                                    <li><strong>Total Amount:</strong> <span>₹{{ number_format((float)($item->accountLedger->total_amount ?? 0), 2) }}</span></li>
                                    <li><strong>Total Paid:</strong> <span class="status-completed">₹{{ number_format((float)($item->accountLedger->total_paid ?? 0), 2) }}</span></li>
                                    <li><strong>Current Balance:</strong> <span>₹{{ number_format((float)($item->accountLedger->current_balance ?? 0), 2) }}</span></li>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let isMagnifierOn = localStorage.getItem('magnifierState');
    if (isMagnifierOn === null) {
        isMagnifierOn = 'on'; // default on
        localStorage.setItem('magnifierState', 'on');
    }
    ...
    toggleBtn.addEventListener('click', () => {
        ...
        localStorage.setItem('magnifierState', isMagnifierOn);
