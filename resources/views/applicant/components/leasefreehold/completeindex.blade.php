...
@section('title', 'Completed Lease Free Hold - List')
@section('content')
    @if (session('error'))
    ...
    @endif
    @php
        #return getDebugIndex($freeholdAllottee);
    @endphp
        ...
        <div class="compact-card overflow-hidden">
                ...
                <div class="flex items-start justify-between mb-4">
                    <div>
                        <h3 class="flex items-center gap-2 text-sm font-semibold text-gray-700">
                            ...
                            Completed Free Hold Property >> Allottee List
                        </h3>
                    </div>
                </div>
            ...
            <div class="table-responsive">
                <table id="studentListTable" class="table table-striped table-bordered align-middle w-full">
                    ...
                    <tbody id="tableBody">
                        @forelse ($freeholdAllottee as $key => $file)
                            <tr>
                                <!-- SL -->
                                <td>{{ $key + 1 }}</td>
                                ...
                                <td class="py-2">
                                    <div class="flex gap-2">
                                        <a href="{{ route('lease.documents.upload', $file->encrypted_id) }}"
                                            class="action-btn action-btn-info" title="Upload Free Hold Documents">
                ...
                <!-- Pagination -->
                @if ($freeholdAllottee->hasPages())
                    <div id="paginationContainer" class="p-4 border-top">
                        @if (method_exists($freeholdAllottee, 'links'))
                            {{ $freeholdAllottee->links('vendor.pagination.custom') }}
                        @endif
                    </div>
                @endif
            </div>
    ...
    <script>
        (function() {
            ...
            async function loadData(page = 1, searchParams = null) {
                ...
                try {
                    const url = new URL('{{ route('lease.allottee.index') }}');
                    ...
                    if (data.freeholdAllottee?.data?.length) {
                        renderRows(data.freeholdAllottee.data, params, data.freeholdAllottee.current_page, data
                            .freeholdAllottee.per_page);
            ...
            function renderRows(files, searchParams, currentPage, perPage) {
                ...
                <tr>
                    ...
                    <td class="py-2">
                        <div class="flex gap-2">
                            <!-- View -->
                            <a href="/lease-free-hold/documents/${file.encrypted_id}" 
                            class="action-btn action-btn-info" 
                            title="title="Upload Free Hold Documents">
                                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22"
                                    viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                    stroke-linecap="round" stroke-linejoin="round">
