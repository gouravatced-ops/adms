...
@section('content')
    ...
    <div class="app-shell">
        {{-- Header --}}
        <div class="modern-card-header-new">
            <div class="header-flex">
                <div>
                    <h1 class="header-title">New Allottee Assignment Form</h1>
                    <p class="header-subtitle">Enter details of the new allottee for property transfer</p>
                </div>
        ...
        <div class="app-body">
            ...
            <div id="stepContent" class="step-content-area">
                @include('applicant.components.nametransfer.step1')
            </div>
...
@endsection
@php
    #return getDebugIndex($documents);
@endphp
@push('scripts')
    <script>
        // Pass PHP data to JavaScript
        window.documentTransferList = @json($documents);
        window.completedDocumentsList = @json($completedDocuments);
    </script>
    <script>
        const StepManager = {
            config: {
                ...
                // Check if action is 'update' from the applicant data
                isUpdate: {{ isset($applicant) && ($applicant->action ?? '') == 'update' ? 'true' : 'false' }},
                // Also store the action for reference
                action: '{{ isset($applicant) ? $applicant->action ?? 'create' : 'create' }}',
                steps: {
                    1: {
                        create: '{{ route('nametransfer.apply.step1.save') }}',
                        update: '{{ route('nametransfer.apply.step1.update') }}'
                    },
            ...
            init: function() {
                console.log('Initializing with:', {
                    applicantId: this.config.applicantId,
                ...
                @if (isset($applicant) && $applicant->current_step > 1)
                    ...
                    console.log('Loading step:', this.config.currentStep);
                @else
                ...
                // Add hidden field for update identification if in update mode
                if (this.config.isUpdate) {
                    this.addUpdateIdentifier();
                }
            ...
            addUpdateIdentifier: function() {
                // Add hidden input to identify update mode if applicant exists and action is update
                if (this.config.applicantId && this.config.isUpdate) {
                    const form = document.querySelector('#stepContent form');
                    if (form) {
                        // Check if hidden input already exists
                        let hiddenInput = form.querySelector('input[name="is_update"]');
                        if (!hiddenInput) {
                        ...
                        // Add or update applicant_id
                        let applicantInput = form.querySelector('input[name="applicant_id"]');
                        if (!applicantInput) {
                            applicantInput = document.createElement('input');
                            applicantInput.type = 'hidden';
                            applicantInput.name = 'applicant_id';
                            applicantInput.value = this.config.applicantId;
                            form.appendChild(applicantInput);
                        }

                        // Also add action field for reference
                        let actionInput = form.querySelector('input[name="action"]');
                        if (!actionInput) {
                            actionInput = document.createElement('input');
                            actionInput.type = 'hidden';
                            actionInput.name = 'action';
                            actionInput.value = this.config.action;
            ...
            getStepEndpoint: function(step) {
                // Only step 1 has separate create/update endpoints
                if (step === 1) {
                    const step1Config = this.config.steps[1];
                    // Use update endpoint if we're in update mode based on action field
                    if (this.config.isUpdate) {
                        console.log('Using UPDATE endpoint for step 1');
                        return step1Config.update;
                    }
                    console.log('Using CREATE endpoint for step 1');
                    return step1Config.create;
                }

                // For steps 2-5, use the same endpoint regardless of create/update
                console.log(`Using standard endpoint for step ${step}`);
            ...
            loadStep: function(step) {
                ...
                console.log('Loading step from URL:', url);
                ...
                fetch(url, {
                    ...
                    .then(html => {
                        ...
                        // After loading content, add update identifier if in update mode
                        if (this.config.isUpdate) {
                            setTimeout(() => {
                                this.addUpdateIdentifier();
                            }, 100);
        ...
        `;
            ...
            nextStep: function() {
                ...
                // Always include applicant_id if it exists
                ...
                // Add is_update flag based on action
                if (this.config.isUpdate) {
                    formData.append('is_update', 'true');
                }

                // Get the appropriate endpoint based on step
                const endpoint = this.getStepEndpoint(this.config.currentStep);
                ...
                fetch(endpoint, {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            // Update applicant ID if returned from server
                            if (data.applicant_id) {
                                this.config.applicantId = data.applicant_id;
                            }

                            // Update action if returned from server
                            if (data.action) {
                                this.config.action = data.action;
                                this.config.isUpdate = data.action === 'update';
                            }

                            // Add update identifier after successful save if needed
                            if (this.config.isUpdate) {
                                setTimeout(() => {
                                    this.addUpdateIdentifier();
                                }, 100);
            ...
            submitApplication: function() {
                fetch('{{ route('nametransfer.apply.step6.save') }}', {
                        ...
                        body: JSON.stringify({
                            ...
                            action: this.config.action,
                            is_update: this.config.isUpdate
                        })
                    })
                    .then(res => res.json())
                    .then(data => {
                        if (data.success) {
                            ...
                            setTimeout(() => {
                                window.location.href = '{{ route('nametransfer.dataentry.files') }}';
                            }, 2000);
                        } else {
