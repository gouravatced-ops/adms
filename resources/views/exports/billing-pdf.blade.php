    ...
    @php
        ...
        foreach ($chunkSizes as $chunkSize) {
            if ($index < $totalAllottees) {
                $chunk = array_slice($allotteesArray, $index, $chunkSize);
                if (!empty($chunk)) {
                    $chunks[] = $chunk;
                    $pageCounter++;
                }
        ...
        @foreach ($chunks as $pageIndex => $chunk)
            <div class="page-wrapper {{ ($copyIndex > 0 || $pageIndex > 0) ? 'page-break' : '' }}">
                ...
                @if($pageIndex == 0 && $copyIndex == 0)
                    ...
                    <!-- Copy Info - Only on First Page -->
                    <div class="copy-info">
                        <div class="receiving-info">
                            <div class="receiving-date">
                                <span class="info-label">
                                    Total Nos. of Physical Files Received:-
                                </span>
                                {{ $totalFiles }}
                            </div>
                            <div class="receiving-time">
                                <span class="info-label">Total Nos. of Physical Files Scanned:-</span>
                                {{ $totalFiles }}
                            </div>
                        </div>
                    </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse($chunk as $rowIndex => $allottee)
                            <tr>
                ...
                <!-- FOOTER SECTION - Visible on every page with both signatures -->
                <div class="footer">
