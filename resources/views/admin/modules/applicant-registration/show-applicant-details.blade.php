{{-- {{ dd($applicantApplication[0]->acknowledgment_no);}} --}}
                            ...
                            <div class="card-body mt-3 table-responsive">
                                <table border="0" id="application-table" width="100%" cellpadding="5"
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            {{ strtoupper($applicantApplication[0]->passing_state == 'Jharkhand' ? $applicantApplication[0]->passing_state : $applicantApplication[0]->pay_other_state_name) }}
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ strtoupper($applicantApplication[0]->course_name) != 'OTHER' ? strtoupper($applicantApplication[0]->course_name) : strtoupper($applicantApplication[0]->course_name) . ' - ' . strtoupper($applicantApplication[0]->other_course) }}
                                        </td>
                                    </tr>
                                    ...
                                    @else
                                        <tr>
                                            <td><strong>Institute</strong></td>
                                            <td> {{ !empty($applicantApplication[0]->institute_name) ? strtoupper($applicantApplication[0]->institute_name) : strtoupper($applicantApplication[0]->college_name) }}
                                            </td>
                                        </tr>
                                    @endif
                                </table>
                        ...
                        <div class="card">
                            ...
                            <form action="{{ route('payment-action') }}" method="post">
                                ...
                                <div class="card-body mt-3">
                                    ...
                                    {{-- <img id="receipt-image" src="{{ asset($applicantApplication[0]->payment_receipt) }}"
                                    ...
                                    @if (in_array(strtolower($fileExtension), ['jpg', 'jpeg']))
                                        <!-- Display image -->
                                        <img id="receipt-image" src="{{ asset($receiptPath) }}" width="auto"
                                            height="600px" alt="Payment Receipt" />
                                    ...
                                    <div class="d-flex justify-content-between mt-2">
                                        ...
                                        <button type="button" id="revert_pay" class="btn btn-danger">Revert Back</button>
                                    </div>
                                    <div id="reasonInput" class="mt-2" style="display: none;">
                                        <input type="text" name="reason_pay" class="form-control" id="incompleteReason"
                                            placeholder="Enter reason for Revert Back">
                                        <button type="submit" name="revert_pay" class="btn btn-danger mt-2">Submit</button>
                                    </div>
                                </div>
                            </form>
                    ...
                    <div class="col-md-7">
                        <div class="card">
                            ...
                            <div class="card-body mt-3">
                                ...
                                <img id="applicant_preview" src="" data-zoom-image="" width="auto"
                                    height="600px" />
                                ...
                                <div class="d-flex justify-content-between mt-2">
                                    ...
                                    <button type="button" class="btn btn-danger" id="incompleteBtn"
                                        style="display: none;">Revert Back</button>
                                </div>
                                <div id="appReasonInput" class="mt-2" style="display: none;">
                                    <input type="text" class="form-control" id="revertReason"
                                        placeholder="Enter reason for revert back">
                                    <button type="submit" name="next" class="btn btn-danger mt-2"
                                    ...
                                    <!-- {{ $item->document_name }} -->
                                        @switch($item->document_name)
                                            ...
                                            @case('applicant_signature')
                                                <button type="button"
                                                    class="btn {{ $item->status == 'Revert' ? 'btn-danger' : ($item->status == 'Accept' ? 'btn-success' : 'btn-primary') }} mb-2 document-btn"
                                                    data-document="applicant_signature"
                                            ...
                                            @case('state_council_noc')
                                                <button type="button"
                                                    class="btn {{ $item->status == 'Revert' ? 'btn-danger' : ($item->status == 'Accept' ? 'btn-success' : 'btn-primary') }} mb-2 document-btn"
                                                    data-document="state_council_noc"
                                                    data-src="{{ asset($item->document_path) }}">
                                            ...
                                            @case('tenth_certificate')
                                                <button type="button"
                                                    class="btn {{ $item->status == 'Revert' ? 'btn-danger' : ($item->status == 'Accept' ? 'btn-success' : 'btn-primary') }} mb-2 document-btn"
                                                    data-document="tenth_certificate"
                                                    data-src="{{ asset($item->document_path) }}">
                                            ...
                                            @case('twelfth_certificate')
                                                <button type="button"
                                                    class="btn {{ $item->status == 'Revert' ? 'btn-danger' : ($item->status == 'Accept' ? 'btn-success' : 'btn-primary') }} mb-2 document-btn"
                                                    data-document="twelfth_certificate"
                                                    data-src="{{ asset($item->document_path) }}">
                                            ...
                                            @case('caste_certificate')
                                                <button type="button"
                                                    class="btn {{ $item->status == 'Revert' ? 'btn-danger' : ($item->status == 'Accept' ? 'btn-success' : 'btn-primary') }} mb-2 document-btn"
                                                    data-document="caste_certificate"
                                                    data-src="{{ asset($item->document_path) }}">
                                                    Caste Certificate
                                                </button>
                                            @break
                    ...
                    <table border="1" id="preview-table" width="100%" class="table table-striped table-bordered"
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td style="padding: 10px;">
                                    <span
                                        class="badge rounded-pill {{ $applicantApplication[0]->doc_status == 'Accept' ? 'bg-success' : ($applicantApplication[0]->doc_status == 'Revert' ? 'bg-danger' : 'bg-secondary') }}">{{ $applicantApplication[0]->doc_status }}</span>
                                </td>
                                <td style="padding: 10px;">
                                    <span>{{ !empty($applicantApplication[0]->revert_reason) ? $applicantApplication[0]->revert_reason : '----' }}</span>
                                </td>
                            </tr>
                            @php
                                $isAcceptAll = true;
                            ...
                            @foreach ($applicantApplication as $index => $doc)
                                @php
                                    ...
                                    if ($doc->status == 'Pending') {
                                        $isPending = true;
                                        //echo  $applicantApplication[0]->acknowledgment_no=='2409420293' ? $doc->document_name .'-->'. $doc->id : '' ;
                                    }
                                @endphp
                                <tr>
                                    <td style="padding: 10px;">
                                    ...
                                    <td id='docStatus' style="padding: 10px;">

                                        @switch($doc->document_name)
                                            @case('applicant_photo')
                                                <span
                                                    class="badge rounded-pill {{ $doc->status == 'Revert' ? 'bg-danger' : ($doc->status == 'Accept' ? 'bg-success' : 'bg-secondary') }}">{{ $doc->status }}</span>
                                            @break
                    ...
                    <div class="d-flex justify-content-between mt-3">
                        ...
                        @if (auth('admin')->user()->role == 'council_office')
                            <button type="submit" name="verified"
                                class="btn btn-success {{ $isAcceptAll ? '' : 'd-none' }}"
                                {{ $isPending ? 'disabled' : '' }} id="verifyBtn">Accept</button>
                        @endif
                        @if (auth('admin')->user()->role == 'registar')
                            <button type="submit" name="verified"
                                class="btn btn-success {{ $isAcceptAll ? '' : 'd-none' }}"
                                {{ $isPending ? 'disabled' : '' }} id="verifyBtn">Approve</button>
                        @endif

                        <button type="submit" class="btn btn-warning mx-2"
                            style="{{ $isAcceptAll ? 'display: none' : '' }}" name="incompleteBtn"
                            {{ $isPending ? 'disabled' : '' }} id="incompleteBtn">Incomplete</button>

                        <button type="button" class="btn btn-danger" style="{{ $isAcceptAll ? 'display: none' : '' }}"
                            name="reject" {{ $isPending ? 'disabled' : '' }} id="reject">Reject</button>

                        <button type="submit" class="btn btn-danger mx-2" name="rejectBtn"
                            {{ $isPending ? 'disabled' : '' }} id="rejectBtn" style="display: none">Reject</button>

                        <input type="text" name="reject_reason" class="form-control" id="reject_reason"
                            {{ $isPending ? 'disabled' : '' }} style="display: none" disabled
