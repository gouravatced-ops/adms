...
@section('admin-content')
    <div class="container-xxl flex-grow-1 ">
        <h6 class="py-3 mb-4"><span class="invert-text-white">Registration Certificate / View Accepted Aplicant List </span>
        </h6>

        <div class="card mb-4">
            <h5 class="card-header text-white bg-info">View Accepted Applicant List</h5>
            @if (session('success'))
                        ...
                        <tbody>
                            @foreach ($regisStuList as $key => $stuList)
                                <tr>
                                    ...
                                    @endif
                                    <td>{{ $stuList->accepted_datetime }}</td>
                                    <td>{{ $stuList->user }}</td>
                                    <td><a href="{{ route('applicant_af.pdf', $stuList->acknowledgment_no) }}"><svg
                                                xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                                fill="none" stroke="#f13c0e" stroke-width="2" stroke-linecap="round"
                                    ...
                                    <td>
                                        <a href="{{ route('edit.applicant-form-dtl', $stuList->acknowledgment_no) }}"
                                            class="btn btn-primary">Edit</a>
                                        <br><br>
                                        <!-- <form action="{{ route('application-mark-pending', $stuList->acknowledgment_no) }}"
                                            method="post" id="markPendingForm">
                                            @csrf
                                            @method('PUT')
                                            <button type="button" class="btn btn-warning" id="markPending">Mark Pending</button>
                                        </form> -->

                                        <form
                                            action="{{ route('application-mark-pending', $stuList->acknowledgment_no) }}"
                                            method="post" id="markPendingForm-{{ $stuList->acknowledgment_no }}">
                                            @csrf
                                            @method('PUT')
                                            <button type="button" class="btn btn-warning markPending"
                                                data-ack="{{ $stuList->acknowledgment_no }}"
                                                data-form-id="markPendingForm-{{ $stuList->acknowledgment_no }}">Mark
                                                Pending</button>
                                        </form>
...
@push('script')
    ...
    <script>
        document.querySelectorAll('.markPending').forEach(button => {
            button.addEventListener('click', function () {
                const acknowledgmentNo = this.dataset.ack;
                const formId = this.dataset.formId;

                if (confirm(`Do you want to mark Acknowledgement No. ${acknowledgmentNo} as pending?`)) {
