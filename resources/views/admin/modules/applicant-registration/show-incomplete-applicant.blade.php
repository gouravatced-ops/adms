...
@section('admin-content')
    <div class="container-xxl flex-grow-1 ">
        <h6 class="py-3 mb-4"><span class="invert-text-white">Registration Certificate / View Incompleted Aplicant List
            </span>
        </h6>

        <div class="card mb-4">
            <h5 class="card-header text-white bg-info">View Incompleted Applicant List</h5>
            @if (session('success'))
                        ...
                        <tbody>
                            @foreach ($regisStuList as $key => $stuList)
                                <tr>
                                    ...
                                    @endif
                                    <td>{{ empty($stuList->pay_revert) ? $stuList->doc_names : 'payment_receipt, '. $stuList->doc_names }}</td>
                                    <td>{{ $stuList->stu_revert_date == null ? 'NA' : $stuList->stu_revert_date }}</td>
                                    <td>
                                        <a title="View" class=" text-info"
                                            href="{{ route('incomplete-applicant-details', ['id' => $stuList->acknowledgment_no]) }}"><i
