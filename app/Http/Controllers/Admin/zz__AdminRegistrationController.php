...
class AdminRegistrationController extends Controller
{
    ...
    public function viewEditApplicantFormDetails($id)
    {
        ...
        return view('admin.modules.applicant-registration.show-edit-applicant-form-details', compact('application', 'payment'));
    ...
    public function getApplicantDetail($id)
    {
        $applicantApplication = StudentApplication::query()
            ...
            ->select('student_applications.*', 'payments.*', 'uploaded_documents.*', 'institutes.name as institute_name', "courses.course_name", "indian_states.name as pay_other_state_name", "res_state.name as res_other_state_name")
            ->get();

        return view("admin.modules.applicant-registration.show-applicant-details", compact("applicantApplication"));
    ...
    public function getAcceptedApplicantDetail($id)
    {
        ...
        return view("admin.modules.registar.show-applicant-details", compact("applicantApplication"));
    ...
    public function getIncompleteApplicantDetail($id)
    {
        $applicantApplication = StudentApplication::query()
            ...
            ->select('student_applications.*', 'payments.*', 'uploaded_documents.*', 'institutes.name as institute_name', "courses.course_name", "indian_states.name as pay_other_state_name", "res_state.name as res_other_state_name")
            ->get();
        return view("admin.modules.applicant-registration.show-incomplete-applicant-details", compact("applicantApplication"));
    ...
    public function applicationChangeStatus(Request $request)
    {
        ...
        if ($request->has('verified')) {
            ...
            if ($stuApp) {
                ...
                return redirect()->route('view.pending-registration')->with('success', 'Application status accepted successfully!');
            ...
            if ($stuApp) {
                ...
                return redirect()->route('view.pending-registration')->with('success', 'Application status incomplete successfully!');
            ...
            return redirect()->route('view.pending-registration')->with('success', 'Application status rejected successfully!');
        }
    }
    ...
    public function applicationChangeStatusByRegistar(Request $request)
    {
        ...
        if ($request->has('verified')) {
            ...
            if ($stuApp) {
                $message = "Dear {$stuApp->name}, your application is approved.-JSPCRN";
                // $this->sendSMS($stuApp->mobile_no, $message); // As per Council Request This Message has been disabled.
                // $this->sendSMS('7979040859', $message);
    ...
    public function generateCertificate(Request $request)
    {
        ...
        if ($otpLog) {
            ...
            if (count($stuApp) > 0) {
                ...
                $lastRecord = ApprovedApplicants::whereNotNull('alloted_regn_no')
                    ->orderBy('id', 'desc')
                    ->first();

                // Extract the last five digits from the last code
                if ($lastRecord) {
                    $lastCode = $lastRecord->alloted_regn_no;
                    $lastFiveDigits = intval(substr($lastCode, -5)); // Get the last 5 digits
                } else {
                    // If there is no previous code, start from 11769
                    $lastFiveDigits = 11768;
                }
                // Increment the last five digits by 1
                $newNumber = $lastFiveDigits + 1;

                // $registrationNo = $stuApp[0]->course_code . '' . str_pad($stuApp[0]->inst_code, 3, '0', STR_PAD_LEFT) . '' . $newNumber;

                $registrationNo = $newNumber;
                ...
                if ($pdf->save($fullFilePath)) {
                    $student = ApprovedApplicants::updateOrCreate(
                        ['student_application_id' => $request->stu_app_id],
                        [
                            'allotted_certificate_no' => $certificateNo,
                            'alloted_regn_no' => $registrationNo,
                            'certificate_path' => $fullFilePath,
                            'certficate_valid_from_date' => now()->format('Y-m-d H:i:s'),
                            'created_at' => auth('admin')->user()->id,
                        ]
                    );
                }
                return back()->with('success', 'Applicant Certificate No. generated successfully!');
            }
        } else {
